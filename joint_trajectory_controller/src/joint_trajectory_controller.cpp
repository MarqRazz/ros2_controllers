  ...
  const std::shared_ptr<rclcpp_action::ServerGoalHandle<FollowJTrajAction>> goal_handle)
{
  ...
  if (active_goal && active_goal->gh_ == goal_handle)
  {
    ...
    {
      // The robot was tracking when the cancel arrived, so hold the last commanded position to
      // keep the command stream continuous. Fault paths deliberately keep using the measured
      // state.
